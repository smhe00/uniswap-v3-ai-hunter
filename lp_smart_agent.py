...
# --- CONFIGURATION & ROBUST WIDE-RANGE PARAMETERS ---
MODEL_PATH = 'v3_experimental_15m_tag/models_15m.pkl'
STATE_FILE = 'agent_state.json'
PROXY = {'http': 'http://127.0.0.1:7897', 'https': 'http://127.0.0.1:7897'}

# Optimized Wide-Range Parameters (Alpha +47.65% verified in bear market)
RANGE_PCT = 0.0813          # Golden wide range +/- 8.13%
REBALANCE_DELAY_DAYS = 4    
XGB_RISK_THRESHOLD = 0.57   # Balanced risk sensitivity
MACRO_BULL_RSI = 52         # Robust bull regime detection
MACRO_BEAR_RSI = 50         # Robust bear regime detection

def get_dual_engine_signals():
    print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Fetching MTF Data (15m & 4H)...")
    try:
        ...
        vol_guard_ok = bool(curr['NATR_14'] < 2.0)
...
def run_pulse_15m():
    ...
    if old_mode == 'ACTIVE' and new_mode == 'SAFE':
        if metrics['is_bull']: action = "EXIT -> HOLD ETH"
        elif metrics['is_bear']: action = "EXIT -> HOLD USDC"
        else: action = "EXIT -> KEEP RATIO"
        os.system(f"osascript -e 'display notification \"{action}\" with title \"🛡️ Robust Agent\"'")
    elif (new_mode == 'ACTIVE'):
        if old_mode != 'ACTIVE':
            action = f"RE-ENTER POOL (±{RANGE_PCT*100:.2f}% @ {current_price:.2f})"
            state['last_rebalance'] = now_ts
            os.system(f"osascript -e 'display notification \"Market Safe. Deploying wide range.\" with title \"✅ Robust Agent\"'")
    ...
    print(f"--- ROBUST DUAL-ENGINE PULSE COMPLETE ---")
    print(f"Price: {current_price:.2f} | Macro RSI: {metrics['macro_rsi']:.1f} | Mode: {new_mode}")
    print(f"Action: {action} | Range: ±{RANGE_PCT*100:.2f}%")
